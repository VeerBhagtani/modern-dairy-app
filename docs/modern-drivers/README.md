...
$EDITOR legal/drivers/config.js      # set API_BASE
...
be identical in three places — `legal/drivers/config.js`,
...
```bash
$EDITOR legal/drivers/config.js     # API_BASE for this environment
...
| What | Where |
|---|---|
| Backend URL for the APK | `DRIVERS_API_BASE` secret → `driver-app/www/config.js` |
| Backend URL for the dashboard | `API_BASE` in `legal/drivers/config.js` |
