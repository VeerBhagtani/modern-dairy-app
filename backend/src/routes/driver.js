...
const { enrolLimiter, gpsIngestLimiter, writeLimiter } = require('../middleware/rateLimit');
...
// Shown in the app before the driver accepts enrolment, and again on the main
...
// POST /driver/enrol { name, code, deviceId, appVersion }
// The driver types their name and the one-time code the office gave them.
router.post('/enrol', enrolLimiter, async (req, res) => {
  const { name, code, deviceId, appVersion } = req.body || {};
  if (hasForbiddenKeys(req.body)) return res.status(400).json({ success: false, message: 'Invalid request' });
  if (!isBoundedString(name, { min: 1, max: 80 })) return res.status(400).json({ success: false, message: 'Please enter your name.' });
  if (!isBoundedString(code, { min: 4, max: 32 })) return res.status(400).json({ success: false, message: 'Please enter the code the office gave you.' });
  ...
  const withDash = `${normalised.slice(0, 4)}-${normalised.slice(4)}`;
  const found = await repo.redeemEnrolmentCode(withDash, deviceId);

  // One message for every failure. A distinct "expired" reply would confirm to
  // a guesser that they had found a real code.
  if (!found || found.error) {
    await repo.writeEvent({ kind: 'enrolment_failed', detail: { reason: found?.error || 'not_found', deviceId } });
    return res.status(401).json({ success: false, message: 'That code is not valid. Ask the office for a new one.' });
  }

  // The name the driver types is recorded as their own statement of who they
  // are; the office's record stays authoritative. A mismatch is surfaced to the
  // office rather than silently overwriting either one.
  const patch = { lastSeenAt: Date.now(), appVersion: appVersion || null };
  if (String(name).trim().toLowerCase() !== String(found.name || '').trim().toLowerCase()) {
    patch.selfReportedName = String(name).trim().slice(0, 80);
    await repo.writeEvent({ driverId: found.id, kind: 'name_mismatch', detail: { entered: patch.selfReportedName, onRecord: found.name } });
  }
  await repo.C.drivers().doc(found.id).update(patch);

  const tokens = await issueDriverTokens(found.id, deviceId);
  const { config } = await repo.getConfig();
  res.json({
    success: true,
    data: {
      driver: { id: found.id, name: found.name, driverCode: found.driverCode },
      ...tokens,
...
router.post('/refresh', enrolLimiter, async (req, res) => {
  ...
  try {
    ...
    res.status(401).json({ success: false, message: 'Please enter your code again.' });
  }
});
