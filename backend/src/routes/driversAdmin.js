// Admin API for Modern Drivers. Mounted at /admin/drivers, behind requireAdmin().
//
...
//   viewer   read only
//
// Roles live in drivers_admins/{adminId}. An admin id with no document gets the
// default role below, so the existing single-owner setup keeps working without
// a migration step, and adding staff is one document each.
...
async function roleOf(adminId) {
  const doc = await db.collection('drivers_admins').doc(String(adminId)).get();
  const role = doc.exists ? doc.data().role : null;
  return RANK[role] ? role : DEFAULT_ROLE;
}

function requireRole(min) {
  return async (req, res, next) => {
    try {
      const role = await roleOf(req.adminId);
      req.adminRole = role;
      ...
      res.status(500).json({ success: false, message: 'Could not check permissions' });
    }
  };
}
...
// GET /admin/drivers/dashboard — the fleet list plus today's headline numbers.
...
router.post('/create', requireRole('admin'), writeLimiter, async (req, res) => {
  const body = pickAllowed(req.body, ['name', 'phone', 'driverCode', 'vehicleId']);
  if (!isBoundedString(body.name, { min: 1, max: 80 })) return bad(res, 'A driver name is required');
  if (!isValidId(body.driverCode)) return bad(res, 'A driver ID is required (letters, digits, - and _ only)');
  if (!isOptionalBoundedString(body.phone, { max: 20 })) return bad(res, 'phone is too long');
  if (body.vehicleId != null && !isValidId(body.vehicleId)) return bad(res, 'vehicleId is invalid');
  try {
    const created = await repo.createDriver(body, req.adminId);
    res.json({
      success: true,
      data: {
        ...created,
        note: 'Give this enrolment code to the driver. It is shown once and cannot be retrieved again — generate a new one if it is lost.',
      },
    });
  } catch (e) {
    if (e.code === 'DUPLICATE_CODE') return res.status(409).json({ success: false, message: e.message });
    throw e;
  }
});
...
router.patch('/:driverId', requireRole('admin'), writeLimiter, async (req, res) => {
  ...
  // Mass-assignment guard: status, enrolment, deviceId and activeRideId are NOT
  ...
  res.json({ success: true, data: repo.pickSafe(updated) });
});
...
router.post('/:driverId/enrolment-code', requireRole('admin'), writeLimiter, async (req, res) => {
  const { driverId } = req.params;
  if (!isValidId(driverId)) return bad(res, 'Invalid driver id');
  const driver = await repo.getDriver(driverId);
  if (!driver) return res.status(404).json({ success: false, message: 'Driver not found' });
  const out = await repo.regenerateEnrolmentCode(driverId, req.adminId);
  res.json({ success: true, data: { ...out, note: 'The previous code and any phone already enrolled on it stop working immediately.' } });
});
...
router.post('/maintenance/run', requireRole('admin'), writeLimiter, async (req, res) => {
  const { runMaintenance } = require('../jobs/driversMaintenance');
