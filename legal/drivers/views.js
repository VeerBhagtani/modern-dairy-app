...
window.DRIVERS_VIEWS = (function () {
  ...
  function renderDrivers() {
    return API.drivers(true).then(function (list) {
      set('<div class="card"><h2>Add a driver</h2>'
        + '<div class="row3">'
        + '<div class="field"><label for="dName">Name</label><input id="dName" placeholder="Ramesh Patil"></div>'
        + '<div class="field"><label for="dCode">Driver ID</label><input id="dCode" placeholder="MD-014"></div>'
        + '<div class="field"><label for="dPhone">Phone (optional)</label><input id="dPhone" placeholder="9876543210"></div>'
        + '</div>'
        + '<p id="dErr" class="err" hidden></p><p id="dOk" class="ok-msg" hidden></p>'
        + '<button class="btn-primary" id="btnAdd">Add driver</button></div>'

        + '<div class="card"><h2>Drivers (' + list.length + ')</h2><div style="overflow-x:auto"><table><thead><tr>'
        + '<th>Driver ID</th><th>Name</th><th>Phone</th><th>Vehicle</th><th>Status</th><th>App</th><th></th></tr></thead><tbody>'
        + list.map(function (d) {
          return '<tr>'
            + '<td><b>' + esc(d.driverCode) + '</b></td>'
            + '<td>' + esc(d.name) + (d.selfReportedName ? '<br><span class="tiny">typed "' + esc(d.selfReportedName) + '" in the app</span>' : '') + '</td>'
            + '<td>' + esc(d.phone || '—') + '</td>'
            + '<td>' + esc(d.vehicleId || '—') + '</td>'
            + '<td><span class="pill ' + (d.status === 'active' ? 'active' : 'idle') + '">' + esc(d.status) + '</span></td>'
            + '<td>' + (d.enrolmentPending ? '<span class="pill warn">not set up</span>' : '<span class="pill live">set up</span>') + '</td>'
            + '<td style="white-space:nowrap">'
            + '<button class="btn-outline btn-sm" data-code="' + esc(d.id) + '">New code</button> '
            + '<button class="btn-outline btn-sm" data-toggle="' + esc(d.id) + '" data-status="' + esc(d.status) + '">' + (d.status === 'active' ? 'Deactivate' : 'Reactivate') + '</button>'
            + '</td></tr>';
        }).join('')
        + '</tbody></table></div></div>');
      ...
      on('#btnAdd', 'click', function () {
        var body = {
          ...
          driverCode: document.getElementById('dCode').value.trim(),
          phone: document.getElementById('dPhone').value.trim() || undefined,
        };
        var err = document.getElementById('dErr'); err.hidden = true;
        API.createDriver(body).then(function (out) {
          modal('<h3>Driver added</h3>'
            + '<p class="muted">Give this code to ' + esc(body.name) + '. They type it once, with their name, in the Modern Drivers app.</p>'
            + '<p style="font-size:2rem;font-weight:800;letter-spacing:.1em;text-align:center;margin:18px 0">' + esc(out.enrolmentCode) + '</p>'
            + '<p class="tiny">' + esc(out.note) + ' It expires on ' + dateTime(out.expiresAt) + '.</p>'
            + '<button class="btn-primary" id="mOk">Done</button>');
          on('#mOk', 'click', function () { closeModal(); render(); }, document.getElementById('modal'));
        }).catch(function (e) { err.textContent = e.message; err.hidden = false; });
      });
      on('[data-code]', 'click', function (e) {
        if (!confirm('Issue a new enrolment code? The old code stops working and the driver\'s current phone is signed out.')) return;
        API.newEnrolmentCode(e.currentTarget.dataset.code).then(function (out) {
          modal('<h3>New code</h3><p style="font-size:2rem;font-weight:800;letter-spacing:.1em;text-align:center;margin:18px 0">' + esc(out.enrolmentCode) + '</p>'
            + '<p class="tiny">' + esc(out.note) + '</p><button class="btn-primary" id="mOk">Done</button>');
          on('#mOk', 'click', function () { closeModal(); render(); }, document.getElementById('modal'));
        }).catch(function (err2) { alert(err2.message); });
      });
      on('[data-toggle]', 'click', function (e) {
        var next = e.currentTarget.dataset.status === 'active' ? 'inactive' : 'active';
        API.setDriverStatus(e.currentTarget.dataset.toggle, next).then(render).catch(function (err2) { alert(err2.message); });
      });
    });
  }
  ...
  function renderPlaces() {
    return Promise.all([API.places('restaurants'), API.places('facilities')]).then(function (r) {
      ...
      on('#btnExport', 'click', function () {
        API.download('/admin/drivers/restaurants/export.csv', {}, 'restaurants.csv').catch(function (e) { alert(e.message); });
      });
  ...
  function exportReport(format) {
    ...
    API.download('/admin/drivers/reports/' + name, params, name + '.' + (format === 'xls' ? 'xls' : 'csv'))
