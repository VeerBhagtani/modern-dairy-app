 ...
 *
 * Every call carries the signed-in admin's Firebase ID token. The backend
 * verifies it properly (signature, expiry, audience, issuer) and then checks
 * the uid, so the dashboard holds no second credential of its own.
 *
 * window.DRIVERS_AUTH.getToken is installed by app.js once Firebase Auth is up.
 */
window.DRIVERS_API = (function () {
  ...
  return {
    ...
    createDriver: function (body) { return request(D + '/create', { method: 'POST', body: body }); },
    ...
    newEnrolmentCode: function (id) { return request(D + '/' + id + '/enrolment-code', { method: 'POST', body: {} }); },
